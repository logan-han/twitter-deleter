        ...
        if (result.Items[0]) {
            ...
            if(tweet_ids.length == 0) {
                ...
                dynamoDb.delete(params, function(error, result) {
                    if (error) {
                        console.error("Unable to delete item:", JSON.stringify(error, null, 2));
                    } else {
                ...
                dynamoDb.update(params, function(error, result) {
                    if (error) {
                        console.error("Unable to update item:", JSON.stringify(error, null, 2));
                    } else {
            ...
            for(i in to_delete_list) {
                oauth.post("https://api.twitter.com/1.1/statuses/destroy/" + to_delete_list[i] + ".json", twitter_token, twitter_secret, {}, function (error, data, response) {
                    if (error) {
                        console.log(error);
                    }
                });
